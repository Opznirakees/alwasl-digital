...
export function handleApiError(error: unknown) {
  ...
  if (error instanceof Error) {
    ...
    if (error.message === 'PAYMENT_PROVIDER_NOT_CONFIGURED') return fail('Payment provider is not configured', 424);
    if (error.message === 'WAHO_PROVIDER_NOT_CONFIGURED') return fail('WAHO provider is not configured', 424);
