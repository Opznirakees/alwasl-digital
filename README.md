...
Set all `QICARD_*` values as encrypted runtime secrets. `APP_BASE_URL` must be the public HTTPS application origin so QiCard can reach `/api/webhooks/qicard`. `X-Terminal-Id` is mandatory and must match the configured terminal. Every accepted callback is deduplicated, audited without storing card data, and confirmed through QiCard's authenticated status API before any order mutation.
