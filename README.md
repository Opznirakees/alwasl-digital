...
`QICARD_WEBHOOK_MODE=onboarding` is a deliberately isolated acceptance mode. It accepts only `https://uat-sandbox-3ds-api.qi.iq/api/v1`, requires checkout to remain disabled, and validates callback data against the authenticated sandbox payment before returning `200`. Verified QiCard test payments without a local order are stored as `PROVIDER_TEST_VERIFIED` and can never alter orders, wallets, revenue, or WAHO fulfillment. Moving to `QICARD_WEBHOOK_MODE=production` requires merchant production credentials, a production terminal/API URL, and the matching RSA public key; production callbacks require both RSA-SHA256 signature validation and an authenticated QiCard API match.
