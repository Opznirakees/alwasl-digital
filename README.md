...
Set all `QICARD_*` values as encrypted runtime secrets. `APP_BASE_URL` must be the public HTTPS application origin so QiCard can reach `/api/webhooks/qicard`. `X-Terminal-Id` is mandatory for merchant API calls. QiCard callbacks are authenticated with `X-Signature`; an optional callback `X-Terminal-Id` is also checked when present. Every accepted callback is deduplicated, audited without storing card data, and confirmed through QiCard's authenticated status API before any order mutation.

`QICARD_WEBHOOK_MODE=onboarding` is a deliberately isolated acceptance mode. It accepts only `https://uat-sandbox-3ds-api.qi.iq/api/v1` and validates callback data against the authenticated sandbox payment before returning `200`. `QICARD_ENABLED=true` can expose the hosted QiCard checkout on the dedicated acceptance deployment; keep it false on environments that should receive webhook probes only. Verified QiCard test payments without a local order are stored as `PROVIDER_TEST_VERIFIED` and can never alter orders, wallets, revenue, or WAHO fulfillment. Moving to `QICARD_WEBHOOK_MODE=production` requires merchant production credentials, a production terminal/API URL, and the matching RSA public key; production callbacks require both RSA-SHA256 signature validation and an authenticated QiCard API match.
...
The app exposes `GET /api/health` for external uptime providers. Admins can manage additional uptime targets and log retention in the admin Monitoring tab. Set `MONITORING_ERROR_WEBHOOK_URL` and/or `MONITORING_STATUS_WEBHOOK_URL` to forward events to an external error tracking or incident system. Customer checkout is currently cash-only: keep `QICARD_ENABLED=false`. The QiCard client and signed webhook implementation remain available for a later controlled reactivation after merchant acceptance, but they are not exposed as a customer payment method. Configure `WAHA_BASE_URL`, `WAHA_API_KEY`, and `WAHA_SESSION` for direct WhatsApp OTP delivery. Native WAHO API fulfillment can still be added later; until then, add `WAHO_FULFILLMENT_PHONE` to route paid WAHO top-up requests to the operator WhatsApp queue. Provider accounts are seeded with priority/fallback metadata, an initial routing balance from `WAHO_PROVIDER_INITIAL_BALANCE`, and a low-balance threshold from `WAHO_PROVIDER_LOW_BALANCE_THRESHOLD`; set `PROVIDER_ALERT_WHATSAPP_PHONE` to receive WhatsApp alerts. Manage real operational balances carefully after seed. Do not enable local-only demo or fake payment flags in production.
