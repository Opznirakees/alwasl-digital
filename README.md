...
Set all `QICARD_*` values as encrypted runtime secrets. `APP_BASE_URL` must be the public HTTPS application origin so QiCard can reach `/api/webhooks/qicard`. Obtain `QICARD_WEBHOOK_PUBLIC_KEY` directly from Qi; it is not published in the integration guide. `QICARD_ENABLED` controls new customer checkouts only, so signed callbacks for existing payments keep working while checkout is paused. The webhook validates `X-Signature` with RSA-SHA256, validates `X-Terminal-Id` when Qi sends it, deduplicates delivery, and confirms known payments once more through the authenticated Qi API before fulfillment. Valid signed onboarding notifications without a matching local payment are acknowledged without changing an order.
